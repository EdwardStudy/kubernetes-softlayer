...
SSH_ID=k8s_rsa
SSH_PUB_KEY=~/.ssh/${SSH_ID}.pub
SSH_PRI_KEY=~/.ssh/${SSH_ID}
...
function set_ssh_key() {
	...
	sshpass -p $1 ssh-copy-id -i $SSH_PUB_KEY -o 'StrictHostKeyChecking=no' -o 'UserKnownHostsFile=/dev/null' root@$2

	if grep -q "$2" ~/.ssh/config; then
		echo -e "\n\033[32m[INFO] SSH config item is existing.\033[0m"
	else
    echo -e "\n\033[32m[INFO] Write SSH config to ~/.ssh/config.\033[0m"
		cat >>~/.ssh/config <<EOF

Host $2
HostName $2
User root
IdentityFile ${SSH_PRI_KEY}
EOF
	fi
}
...
function configure_masters() {
	...
	ansible-playbook ansible/kube-master.yaml -v -i $HOSTS -e "master_ip=$MASTER1_IP"
}
